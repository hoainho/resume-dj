...
class Social(models.Model):
    ...
    link = models.CharField(max_length=50, validators=[
        RegexValidator(
            regex='^(http[s]?:\\/\\/(www\\.)?|ftp:\\/\\/(www\\.)?|www\\.){1}([0-9A-Za-z-\\.@:%_\+~#=]+)+((\\.[a-zA-Z]{2,3})+)(/(.)*)?(\\?(.)*)?',
            message='Link must be start with http or https',
            code='invalid_format_link'
        )
    ], blank=True, null=True)
    def __str__(self):
        return self.name + " of " + self.user.username if self.user.username else None
...
class UserProfile(models.Model):
    ...
    def get_social(self):
        social = Social.objects.filter(user=self.user)
        return social
...
class EducationItem(models.Model):
    ...
    description = models.CharField(max_length=500)
    def __str__(self):
        return self.position + " at " + self.university
